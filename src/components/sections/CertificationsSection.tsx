      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {certifications.map((cert) => (
            <div
              key={cert.id}
              className="group flex items-center gap-3 px-5 py-3 rounded-xl bg-card border border-border hover:border-teal-accent/30 hover:shadow-md transition-all duration-300"
            >
              {cert.logo_url ? (
                <img src={cert.logo_url} alt={cert.name} className="w-8 h-8 object-contain" />
              ) : (
