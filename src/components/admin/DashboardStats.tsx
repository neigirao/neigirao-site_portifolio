...
import { Briefcase, Code, GraduationCap, FolderOpen, AlertTriangle, CheckCircle, Image, FileText } from 'lucide-react';
...
function calculateStats(items: ContentItem[], imageField: 'logo_url' | 'image_url' = 'logo_url') {
...
export function DashboardStats({ experiences, skills, education, projects, isLoading }: DashboardStatsProps) {
  ...
  const totalItems = experiences.length + skills.length + education.length + projects.length;
  const totalComplete = expStats.complete + skillStats.complete + eduStats.complete + projStats.complete;
  const overallCompletion = totalItems > 0 ? Math.round((totalComplete / totalItems) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {totalComplete} de {totalItems} itens completos
              </p>
            </div>
      ...
      {/* Individual stats */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4" role="list" aria-label="Estatísticas por categoria">
        <div role="listitem">
          <StatCard
            title="Experiências"
            count={experiences.length}
            icon={<Briefcase className="h-4 w-4 text-primary" />}
            complete={expStats.complete}
            incomplete={expStats.missingSeo}
            missingImage={expStats.missingImage}
            isLoading={isLoading}
          />
        </div>
        <div role="listitem">
          <StatCard
            ...
            complete={projStats.complete}
            incomplete={projStats.missingSeo}
            missingImage={projStats.missingImage}
            isLoading={isLoading}
          />
        </div>
        <div role="listitem">
          <StatCard
            ...
            complete={skillStats.complete}
            incomplete={skillStats.missingSeo}
            missingImage={skillStats.missingImage}
            isLoading={isLoading}
          />
        </div>
        <div role="listitem">
          <StatCard
            ...
            complete={eduStats.complete}
            incomplete={eduStats.missingSeo}
            missingImage={eduStats.missingImage}
            isLoading={isLoading}
          />
        </div>
      </div>
